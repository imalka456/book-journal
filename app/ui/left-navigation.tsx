  ...
  return (
    <div>
      <div className="w-full h-16 bg-dark-blue sm:hidden flex items-center px-6">
        ...
        <span className="font-bold text-white block font-bold text-xl ml-4">
          My Book App
        </span>
      </div>

      <div
        className={`ease-in-out duration-500 ${isOpenMobileNav ? "translate-x-0" : "-translate-x-full"}  sm:hidden`}
      >
      ...
      <div className="hidden sm:block">
        <nav className="bg-dark-blue p-5 h-screen max-sm:fixed max-sm:z-10">
          <div
            className={`dark-blue text-white ${isOpen ? "w-64" : "w-20"} p-4 transition-all duration-300 relative`}
          >
            ...
            <div className="inline-flex pb-8">
              <FaBook
                className={`w-8 h-8 flex-shrink-0 text-white text-2xl rounded block cursor-pointer float-left mr-4 duration-500 ${isOpen && "rotate-[360deg]"}`}
              />
              <h1
                className={`font-bold block origin-left text-base font-bold text-xl duration-300 ${!isOpen && "scale-0"}`}
              >
                My Book App
              </h1>
            </div>

...
export default function Table({
  books,
  onEdit,
}: Readonly<{ books: Book[]; onEdit: (book: Book) => void }>) {
  return (
    <div className="mt-6">
              ...
              {books.map((book: Book) => (
                <tr className="border-b" key={book.title}>
                  ...
                  <td className={`text-md px-6 py-3 bg-opacity-50`}>
                    <div
                      className={`m-2 px-2 text-center rounded-lg ${
                  ...
                  <td className="text-md px-6 py-3">
                    <button onClick={() => onEdit(book)}>
                      <MdOutlineEdit className="rounded-full p-1 text-2xl bg-dark-blue text-white cursor-pointer hover:bg-hover-color" />
                    </button>
                  </td>

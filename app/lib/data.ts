...
export async function fetchBooks(): Promise<Book[]> {
  ...
  export async function addNewBook(book: { title: string; isbn: string; startDate: Date | undefined; endDate: Date | undefined; isToggled: boolean}): Promise<void>{
    try {
      ...
      const startDateFormatted = book.startDate ? formatDateToDB(book.startDate) : null;
      ...
      if (!userBook) {
        const { data: userBookRecord, error: insertUserBookError } = await supabase
          .from('user_books')
          .insert([
            {
              user_id: userId,
  ...
  export async function editBook(book: { id: number | undefined; title: string; isbn: string; startDate: Date | undefined; endDate: Date | undefined; isToggled: boolean}): Promise<void>{
    try {
      ...
      if (userBooksError) {
        throw new Error(`Failed to update user_books table: ${userBooksError.message}`);
      }

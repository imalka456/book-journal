...
export default function AddBook({
  ...
  const [formData, setFormData] = useState<{
    ...
    startDate:
      initialData?.start_date && initialData.start_date !== null
        ? new Date(initialData.start_date)
        : undefined, // Can be a Date or null
    endDate:
      initialData?.end_date && initialData.end_date !== null
  ...
  return (
    <div className="popup-overlay">
      <div className="popup-content">
        <div className="flex">
          <div>
            <h2 className="mb-4 text-xl text-active-color font-medium ">
              {mode === "edit" ? "Update My Book Record" : "Add a New Book"}
            </h2>
          </div>
          <div>
            {formData.isToggled && (
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <label className="autoSaverSwitch relative inline-flex cursor-pointer select-none items-center">
            <span className="sr-only">
              {formData.isToggled ? "Toggle Finished" : "Toggle In Progress"}
            </span>
            <input
              type="checkbox"
              name="autoSaver"
            ...
            <span className="">
              <span className="pl-1">
                {" "}
                {formData.isToggled ? "Finished" : "In Progress"}{" "}
              </span>
            </span>
          </label>
